...
def claim(root: Path) -> bool:
    ...
    race for port 8005 and open DuckDB twice, which is a corrupted install rather than a
    slow one.
    """
    global _lock
...
class Tray:
    ...
    the browser leaves it running, holding port 8005 and the catalog lock.
    """
...
def guard_children(root: Path) -> None:
    ...
    *killed* — Task Manager, ``taskkill /F``, a crash — never returns from that wait, and the
    ...
    and port 8005 against every later start, which fails with nothing on screen to explain it.
