...
from .fastexpr import REQUIRED_KEYWORDS, OperatorInfo, operator_table
...
def catalogue(operators: list[dict[str, Any]]) -> Catalogue:
    ...
    for name, info in sorted(operator_table(operators, "REGULAR").items()):
        kind = _kind(info)
        # A search point is written positionally, which these operators refuse.
        if kind is None or name in EXCLUDED or name in REQUIRED_KEYWORDS:
            continue
