...
from . import distributions as blobs
...
class Optimizer:
    ...
    async def _harvest(self, study_id: int, *, tell: bool = True, vault_first: bool = False) -> int:
        ...
        async with self.db.session() as session:
            ...
            blob_bodies = await blobs.bodies(session, [t.distributions for t in open_trials])
            records = {
                r.id: r
                for r in (
                    await session.scalars(
        ...
        if tell:
            ...
            await asyncio.to_thread(_tell_many, study, finished, blob_bodies)
    ...
    async def optuna_study(self, study_id: int, row: Study) -> optuna.Study:
        ...
        async with self.db.session() as session:
            ...
            blob_bodies = await blobs.bodies(session, [t.distributions for t in trials])
            history = [
                {
                    "params": t.params or {},
                    "distributions": blobs.expand(t.distributions, blob_bodies),
                    "values": t.values,
                    "state": t.state,
...
def _tell_many(
    ...
    for optuna_trial, stored, values, summary in finished:
        ...
        try:
            ...
            distributions = _load_distributions(blobs.expand(stored.distributions, blob_bodies))
            study.add_trial(
                create_trial(
