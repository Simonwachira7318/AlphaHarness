...
COMPARISONS = ("==", "!=", "<", "<=", ">", ">=")
#: Keywords BRAIN refuses to leave out although the definition publishes a default for them:
#: ``truncate(x)`` fails with 'Required attribute "maxpercent" must have a value', and the
#: failure cancels every other simulation in its batch. The definitions give no way to tell
#: these from real defaults (``rank(x, rate=2)`` is fine), so they are listed from BRAIN's
#: errors, each with the value its definition publishes.
REQUIRED_KEYWORDS: dict[str, dict[str, float]] = {
    "truncate": {"maxPercent": 0.01},
    "ts_delta_limit": {"limit_volume": 0.1},
}


def with_required(name: str, options: dict[str, Any]) -> dict[str, Any]:
    """``options`` plus any keyword BRAIN requires of ``name`` that it lacks, at its default."""
    given = {key.lower() for key in options}
    missing = {
        key: value
        for key, value in REQUIRED_KEYWORDS.get(name, {}).items()
        if key.lower() not in given
    }
