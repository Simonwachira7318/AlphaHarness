...
class QuarterStanding(Out):
    ...
    submitted: int
    #: BRAIN's own count for today, on the platform's calendar.
    submitted_today: int
...
async def standing(state: State) -> QuarterStanding:
    ...
    platform_today = datetime.now(PLATFORM_TZ).date().isoformat()
    submitted_today = sum(count for day, count in days if day == platform_today)
