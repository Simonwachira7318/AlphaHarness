...
@router.post("/forget")
async def forget(state: State) -> Session:
    """Sign out and delete the saved BRAIN login from the local vault.

    The next sign-in needs the email and password typed again.
    """
...
@router.get(
    ...
    responses={200: {"content": {"image/png": {}}}, 404: {"description": "No picture"}},
)
async def avatar(state: State) -> Response:
    """The account's BRAIN profile picture, or 404 when it has none (the UI shows initials)."""
    found = await state.auth.get_avatar()
    if found is None:
        raise refuse(404, "no_avatar", "BRAIN lists no profile picture for this account.")
    body, media = found
    return Response(
        body,
        media_type=media,
