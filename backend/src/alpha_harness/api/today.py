...
from ..brain.filters import PLATFORM_TZ, platform_midnight
from ..db.models import SimStatus, SimulationRecord, SyncStatus
...
class You(Out):
    ...
    #: BRAIN Genius level (``GOLD``, ``EXPERT``, ``MASTER``, ``GRANDMASTER``); null for none.
    genius_level: str | None
    #: The highest level reached, which can be above the current one.
    best_level: str | None
...
async def _you(state: State, session: Any, stored_email: str | None) -> dict[str, Any]:
    ...
    # Cached for the session, so the greeting and the level cost one BRAIN read between them.
    profile = await state.auth.get_user_profile() if session.authenticated else {}
    ...
    return {
        ...
        "geniusLevel": _level(profile.get("geniusLevel") or profile.get("level")),
        "bestLevel": _level(profile.get("bestLevel")),
    }
...
class TodayResults(Out):
    """How today's simulations ended, over the platform's day, as ``Sent Today`` counts it."""

    completed: int
    errored: int
    cancelled: int
    running: int
    #: Sent, but no id came back; reconciliation settles them.
    unresolved: int
    best: BestToday | None


_OUTCOME: dict[str, str] = {
    SimStatus.COMPLETE: "completed",
    SimStatus.WARNING: "completed",
    SimStatus.ERROR: "errored",
    SimStatus.FAILED: "errored",
    SimStatus.TIMEOUT: "errored",
    SimStatus.CANCELLED: "cancelled",
    SimStatus.PENDING: "running",
    SimStatus.RUNNING: "running",
    SimStatus.ORPHANED: "unresolved",
}


@router.get("/results")
async def results(state: State) -> TodayResults:
    """Today's simulations by outcome, and the best Sharpe among those that completed."""
    start = platform_midnight().astimezone(UTC)
    # Children, not batch parents: a parent is ten simulations' envelope, not a result.
    today_rows = (
        SimulationRecord.is_batch.is_(False),
        SimulationRecord.submitted_at >= start,
    )
    ...
    counts: dict[str, int] = dict.fromkeys(set(_OUTCOME.values()), 0)
    for status, count in by_status.tuples():
        bucket = _OUTCOME.get(str(status))
        if bucket:
    ...
    if alpha_ids:
        ...
        rows = await state.catalog.query(
            "SELECT alpha_id, sharpe, fitness, expression FROM alpha "  # noqa: S608
            f"WHERE alpha_id IN ({placeholders}) AND sharpe IS NOT NULL "
            "ORDER BY sharpe DESC LIMIT 1",
            alpha_ids,
        )
        if rows:
            best = rows[0]
    return TodayResults.model_validate({**counts, "best": best})
