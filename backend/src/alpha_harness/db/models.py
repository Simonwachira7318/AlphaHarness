...
class DistributionBlob(Base):
    """A search distribution stored once and shared by every trial that drew from it.

    A field slot's choices are the whole field catalog, half a megabyte of JSON, and every
    trial of a study draws from the same few; inline, they were nearly all of the database.
    A trial keeps ``"blob:<digest>"`` in their place (see ``labs.distributions``).
    """

    __tablename__ = "distribution_blob"
