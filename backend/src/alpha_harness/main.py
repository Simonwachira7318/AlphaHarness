...
Run with::
    ...
    uv run python -m alpha_harness.dev    # development, UI from Vite, on AH_PORT
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    # Before alphas: both live under /api/alphas/{alpha_id}.
...
class SinglePageApp(StaticFiles):
    ...
    async def check_config(self) -> None:
        # In development Vite serves the UI and ``web`` does not exist. Starlette would raise
        # on every unmatched path, turning a mistyped ``/api/...`` into a 500; a missing
        # directory simply finds no files, so it answers 404 instead.
        if self.directory is not None and await anyio.Path(self.directory).is_dir():
