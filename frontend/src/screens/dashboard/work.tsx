...
/**
 * While anything is on BRAIN: how many cores are simulating, and a bar filled to that share of
 * the slots, shimmering while it runs. Gone when nothing is in flight.
 */
function LiveStrip({ status }: { status: EngineStatus }) {
  const cores = Object.values(status.inFlight).reduce((sum, n) => sum + n, 0)
  if (cores === 0) return null
  const share = status.slots > 0 ? Math.min(1, status.slotsUsed / status.slots) : 1
  return (
    <div className="flex flex-col gap-2" role="status">
      <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
        <span className="text-body text-ink">
          <span className="num font-medium">{fmt.int(cores)}</span> simulating on BRAIN now
        </span>
        <span className="text-body-compact text-ink-subtle">
          <span className="num">{fmt.int(status.slotsUsed)}</span> of{' '}
          <span className="num">{fmt.int(status.slots)}</span> slots busy
          {status.queuedTotal > 0 && (
            <>
              {' · '}
              <span className="num">{fmt.int(status.queuedTotal)}</span> waiting
            </>
          )}
        </span>
      </div>
      <div className="h-1.5 overflow-hidden rounded-pill bg-surface-3">
        {/* DESIGN.md → Motion: fills move on transform: scaleX(), never width. */}
        <div
          className={cn('h-full origin-left rounded-pill transition-transform', SWEEP)}
