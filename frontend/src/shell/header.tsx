...
function HeaderCores() {
  ...
  const busy = blocks.reduce((sum, block) => sum + (block.holder ? block.span : 0), 0)
  useSampleCores(busy, slots)
  const view = useCoreView()
  const compact = view.style === 'compact'
