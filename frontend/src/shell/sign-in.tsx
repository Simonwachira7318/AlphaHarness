...
function Verification({
  ...
  // A ref, so a re-render of the page does not restart the wait.
  ...
  useEffect(() => {
    ...
    // One question at a time: the next is asked only after the last is answered. The answer
    // that signs in takes seconds, and questions sent meanwhile asked about a spent check.
    const ask = async () => {
      try {
      ...
      timer = setTimeout(ask, VERIFY_EVERY_MS)
    }
    timer = setTimeout(ask, VERIFY_EVERY_MS)
    return () => {
      stopped = true
