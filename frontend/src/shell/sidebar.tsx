...
export function Sidebar({ you, collapsed }: { you: Today['you']; collapsed: boolean }) {
  ...
  const forget = useMutation({
    mutationFn: () => auth.forget(),
    onSuccess: async () => {
      setForgetting(false)
      toast.success('Saved login removed from this computer')
      await endSession()
    },
    onError: (error) => toast.error(errorMessage(error)),
  })
      ...
      <div className="flex flex-col gap-1 border-t border-hairline p-2">
        ...
        <Menu
          ...
          trigger={
            ...
            >
              <Avatar name={name} userId={you.userId} />
              {!collapsed && (
        ...
        >
          Signs you out and deletes your BRAIN email, password and session from this computer. Your
          Alphas, simulations and AI Keys stay. You will type your email and password to sign in
          again.
        </Confirm>
      </div>
    </aside>
