...
/** The backend's port, from AH_PORT in the repository-root .env (see vite.config.ts). */
export const API_PORT: string = import.meta.env['AH_PORT'] || '8005'

function describe(status: number): string {
  if (status === 0)
    return `Cannot reach the Alpha Harness backend. Start it on port ${API_PORT} and try again.`
