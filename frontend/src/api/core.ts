...
export const today = {
  ...
  /** Today's simulations by outcome, and the best Sharpe among them. */
  results: () => http.get<TodayResults>('/api/today/results'),
}
...
export const auth = {
  ...
  /** Sign out and delete the saved email and password from the local vault. */
  forget: () => http.post<Session>('/api/auth/forget'),
