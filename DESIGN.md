...
than 300ms, except `count` below.

- `sweep` — an ink shimmer crossing a market tile while its fields download, and the Dashboard's
  Work in Flight bar and row markers while simulations are on BRAIN.
- `attention` — a slow amber flash on a nav item the consultant has not opened yet.
- `rise` — the Dashboard greeting fading up into place once as the page opens, a line at a time
  (300ms, staggered 80ms). Never replayed when its figures refresh.
- `count` — a Dashboard figure counting up from zero the first time it shows, easing out over
  600ms: the one motion longer than 300ms, since a count any shorter reads as a flicker. Later
  values replace it in place.
